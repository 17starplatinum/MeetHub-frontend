...
export function Header({ account = false, logged = false }) {
  ...
  return (
    <div className="flex flex-wrap gap-5 z-10 justify-between items-start px-16 py-8 w-full border-solid bg-neutral-950 bg-opacity-95 border-[5px] border-indigo-500 border-opacity-40 max-md:px-16 max-md:max-w-full">
      <div
        className="z-10 shadow-sm cursor-pointer"
        onClick={
          logged ? () => navigate("/events/current") : () => navigate("/")
        }
      >
